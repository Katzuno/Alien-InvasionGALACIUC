...
function OnGUI (){
	if(selected){
		...
		if(scopeTexture != null && inScope){
			...
			if(currentMode == fireMode.launcher){
				...
				if (!aiming) { 
					...
					GUI.DrawTexture(position2, crosshairFirstModeVertical); 	//SUS
					GUI.DrawTexture(position3, crosshairFirstModeHorizontal); 	//Left
					GUI.DrawTexture(position4, crosshairFirstModeVertical);		//JOS
				}
			}
		}
		...
		if(firstMode == fireMode.launcher || secondMode == fireMode.launcher){
			GUI.Label (Rect(Screen.width - 200,Screen.height-95,200,80),"Bullets : ");
...
function FireOneBullet (){
	...
	if (Physics.Raycast(pos, dir, hit, range, layerMask)) {
		...
		var SM : ScoreManager = GameObject.Find("ScoreManager").GetComponent("ScoreManager");
		//var Score : ScoreManager = GameObject.Find();
		//var alien: GameObject = hit.collider.transform.root.gameObject;
		if (hit.rigidbody)
		...
		if (hit.collider.tag == "Concrete") {
		    ...
		    var alien: GameObject = hit.collider.transform.root.gameObject;
		    Destroy(alien);
		  //  var enemy : GameObject = hit.collider.transform.root.gameObject;
		    //Debug.Log(enemy.name);
		   // var EH : EnemyHealth = alien.GetComponent("EnemyHealth");
		   // EH.RemoveHealth(20);
		    

		    SM.addScore(10);
		    
		//    var other : EnemyHealth;
		  //  other.RemoveHealth(damage);
		 //   hit.collider.GetComponent("EnemyHealth");
		   // EH.RemoveHealth(damage);
...
function FireOnePellet(){
	...
	if (Physics.Raycast(pos, dir, hit, range, layerMask)) {
		...
		if (hit.collider.tag == "Concrete") {
		 ...
		 Debug.Log("Enemy Hit by Shotgun");
		    Instantiate (Blood, contact, rot);
		    hit.collider.SendMessageUpwards("ApplyDamage", damage, SendMessageOptions.DontRequireReceiver);	
		    var alien: GameObject = hit.collider.transform.root.gameObject;
		    Destroy(alien);
